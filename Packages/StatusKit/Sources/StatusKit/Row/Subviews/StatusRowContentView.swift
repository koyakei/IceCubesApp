...
struct StatusRowContentView: View {
  ...
  var body: some View {
              KTagWithRelationListView(viewModel: KTagWithRelationListViewModel(
                kTagRelations: viewModel.status.kTagRelations ?? KTagRelations.init(addedKTagRelationList: Set(), addingKTagRelationRequestedList: Set(), deletingKTagRelationRequestedList: Set()),
                client: viewModel.client, statusId: viewModel.status.id))
              //TODO これで参照渡しできてるの？
              NavigationLink(destination: KTagSearchAndAddView(  viewModel: KTagWithRelationListViewModel(kTagRelations: viewModel.status.kTagRelations
                                                                                                          ?? KTagRelations.init(addedKTagRelationList: Set(), addingKTagRelationRequestedList: Set(), deletingKTagRelationRequestedList: Set()), client: viewModel.client, statusId: viewModel.status.id), client: viewModel.client)) {
                  Text("追加")
